...
* - Engineer can later override the selected cable from the UI.
 ...
 // --------------------------------------------------------
 //
 // Automatic selection is allowed to evaluate the complete
 // supported range.
 //
 // The current ProfessionalEngineeringCore supports up to
 // 8 parallel runs.
 //
 // Existing feeder.parallelRuns must NOT restrict automatic
 // selection. It is the result, not the search limit.
 //
 ...
 // --------------------------------------------------------
 //
 // Do NOT invent transformer impedance.
 //
 // If the feeder already contains a verified upstream
 // short-circuit current, use it.
 //
 // Otherwise the design remains DATA_REQUIRED.
 //
 ...
 return FeederResult(
  ...
  feeder: FeederDesign
   ...
   /*
  * If the feeder already contains a valid calculated value,
  * use it.
  */
 ...
 /*
  * demandKVA() is already:
  *
  * demandKW / powerFactor
  *
  * Therefore the current must be calculated directly from
  * apparent power.
  *
  * DO NOT divide by power factor again.
  */
 ...
 /*
  * Weighted PF is retained as a design-reference value
  * for the cable voltage-drop calculation.
  *
  * The design current itself is calculated from demandKVA
  * and therefore does not apply PF a second time.
  */
